...
export function Sidebar() {
  const { data, activeSection, setActiveSection, setInputLanguage } = useCvStore();
  const lang = data.metadata.language_version;
  const labels = i18n[lang];

  return (
    <aside className="w-full lg:w-56 shrink-0 border-r border-[rgba(229,229,229,0.1)] p-6 bg-[rgba(255,255,255,0.01)] backdrop-blur-md overflow-y-auto flex flex-col justify-between h-full">
      <div>
      ...
      <div className="mt-8 border-t border-[rgba(255,255,255,0.05)] pt-6">
        <h3 className="type-label-caps text-[var(--color-outline)] mb-3">Language Version</h3>
